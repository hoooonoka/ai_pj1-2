...
class MonteCarloTreeSearchCaptureAgent(BasicCaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    self.setPathToBorder(gameState)
  ...
  def chooseAction(self, state):
    ...
    # assign to defensive : cause error !!!
    if self.index==0 or self.index==1:
      # decide agent mode
    ...
    if self.mode=='Offensive':
      ...
      #       if self.getMazeDistance(state.getAgentPosition(self.index),state.getAgentPosition(self.enemyPacmanIndex))==1:
      #         actions=self.getActions(state)
      #         for action in actions:
      #           nextState=state.generateSuccessor(self.index,action)
      #           if self.getMazeDistance(nextState.getAgentPosition(self.index),state.getAgentPosition(self.enemyPacmanIndex))==0:
      #             return action
      #   if state.getAgentState(self.enemyGhostIndex).isPacman:
      #     if state.getAgentPosition(self.enemyGhostIndex)!=None:
      #       if self.getMazeDistance(state.getAgentPosition(self.index),state.getAgentPosition(self.enemyGhostIndex))==1:
      #         actions=self.getActions(state)
      #         for action in actions:
      #           nextState=state.generateSuccessor(self.index,action)
      #           if self.getMazeDistance(nextState.getAgentPosition(self.index),state.getAgentPosition(self.enemyGhostIndex))==0:
      #             return action
      if self.moveToOneEntrance:
        if state.getAgentPosition(self.index)[0]==self.boundary:
          self.moveToOtherEntrance=True
          self.moveToOneEntrance=False
          self.changeAnotherEntrance(state)
          actionList=self.shortDistanceToBorder[(state.getAgentPosition(self.index),self.nearestBorder)]
          self.pathToBorder=[]
          for action in actionList:
            self.pathToBorder.append(action)
          action=self.pathToBorder[0]
          self.pathToBorder.remove(action)
          return action
        else:
          actions=self.getActions(state)
          for action in actions:
            nextState=state.generateSuccessor(self.index,action)
            if self.getMazeDistance(self.nearestBorder,state.getAgentPosition(self.index))>self.getMazeDistance(self.nearestBorder,nextState.getAgentPosition(self.index)):
              return action
      ...
      if len(self.history)>10:
        if self.isInYourHome(state):
          oldPosition=self.history[len(self.history)-9][0]
          oldTargetFood=self.history[len(self.history)-9][1]
          if self.nearestFood==oldTargetFood:
            if self.getMazeDistance(oldPosition,state.getAgentPosition(self.index))<3:
              if self.getMazeDistance(oldTargetFood,oldPosition)-self.getMazeDistance(self.nearestFood,state.getAgentPosition(self.index))<3:
                self.moveToOneEntrance=True
                actions=self.getActions(state)
                for action in actions:
                  nextState=state.generateSuccessor(self.index,action)
                  if self.getMazeDistance(self.nearestBorder,state.getAgentPosition(self.index))>self.getMazeDistance(self.nearestBorder,nextState.getAgentPosition(self.index)):
      ...
      if not self.isInYourHome(state):
        ...
        for action in actions:
          ...
          if self.getMazeDistance(nextState.getAgentPosition(self.index),self.nearestBorder)<self.getMazeDistance(state.getAgentPosition(self.index),self.nearestBorder):
            return action
      ...
      if lastState!=None:
        ...
        if len(invaders)==0:
          # no invaders
          # go to best border
          distance=self.getMazeDistance(self.nearestBorder,state.getAgentPosition(self.index))
          for action in actions:
            nextState=self.getSuccessor(state,action)
            if not self.isInYourHome(nextState):
              continue
            if self.getMazeDistance(self.nearestBorder,nextState.getAgentPosition(self.index))<distance:
              bestAction=action
              if shortDistanceToBorder>self.getMazeDistance(self.nearestBorder,nextState.getAgentPosition(self.index)):
                shortDistanceToBorder=self.getMazeDistance(self.nearestBorder,nextState.getAgentPosition(self.index))
          return bestAction
        ...
        for invader in invaders:
          ...
          if state.getAgentPosition(invader)!=None:
            ...
            distance=self.getMazeDistance(state.getAgentPosition(invader),state.getAgentPosition(self.index))
            ...
            for border in self.border:
              enemyToBorderDistance=self.getMazeDistance(state.getAgentPosition(invader),border)
            ...
            denfencerToClosestBorderPositionDistance=self.getMazeDistance(state.getAgentPosition(self.index),closestBorderPosition)
            ...
            for action in actions:
              ...
              nextStateDenfencerToClosestBorderPositionDistance=self.getMazeDistance(nextState.getAgentPosition(self.index),closestBorderPosition)
              if nextStateDenfencerToClosestBorderPositionDistance<enemyToBorderShortestDistance:
                if self.getMazeDistance(state.getAgentPosition(invader),nextState.getAgentPosition(self.index))<distance:
                  # near to enemy agent

                  if shortDistanceToEnemy>self.getMazeDistance(state.getAgentPosition(invader),nextState.getAgentPosition(self.index)):
                    shortDistanceToEnemy=self.getMazeDistance(state.getAgentPosition(invader),nextState.getAgentPosition(self.index))
                    bestAction=action
                    bestPosition=state.getAgentPosition(invader)
              else:
                if self.getMazeDistance(closestBorderPosition,nextState.getAgentPosition(self.index))<denfencerToClosestBorderPositionDistance:
                  bestAction=action
                  bestPosition=state.getAgentPosition(invader)
        if bestAction!='Stop':
          nextState=self.getSuccessor(state,action)
          ...
          for border in self.border:
            distance=self.getMazeDistance(self.lastPacmanPosition,border)
          ...
          distanceToLastPacmanPositionShortestPosition=self.getMazeDistance(state.getAgentPosition(self.index),lastPacmanPositionShortestPosition)
          for action in actions:
            nextState=self.getSuccessor(state,action)
            if not self.isInYourHome(nextState):
              continue
            if self.getMazeDistance(lastPacmanPositionShortestPosition, nextState.getAgentPosition(self.index)) < distanceToLastPacmanPositionShortestPosition:
              bestAction=action
  ...
  def simulation(self,startState,startContainFood,state,depth):
    if depth<self.simulationDepth:
      ...
      nextState=state.generateSuccessor(self.index,action)
      if self.getScore(nextState)>self.getScore(state):
        return (120*self.containFood,action,depth+1)
      ...
      if self.powerOn==False:
        if len(oldCapsule)>len(newCapsule):
          ...
          if len(oldCapsule)!=0:
            ...
            if self.getMazeDistance(capsule,state.getAgentPosition(self.index))>self.getMazeDistance(capsule,nextState.getAgentPosition(self.index)):
              # move toward capsule
              return 30*(self.getMazeDistance(capsule,state.getAgentPosition(self.index))-self.getMazeDistance(capsule,nextState.getAgentPosition(self.index)))
        
      if self.getScore(state)<self.getScore(nextState):
          # get higher score <=> storing food in home
          reward+=120*self.containFood
      else:
        ...
        if startContainFood<3:
          ...
          if newBorderDistance<oldBorderDistance:
            # home direction
            reward+=4*self.containFood*(oldBorderDistance-newBorderDistance)
